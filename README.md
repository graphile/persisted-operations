...
then use the `addToPersistedQueries.js` script below to split this queries JSON
file into one file per query that can be passed to
...
For everything except Relay, we recommend that you generate persisted queries
...
We also recommend using the file `addToPersistedQueries.js` to write the
...
You can configure Apollo Client to send the persisted queries generated by
